...
Datafeeds.UDFCompatibleDatafeed.prototype.getServerTime = function(callback) {
  if (this._configuration.supports_time) {
    this._send(this._datafeedURL + "/exchange/time", {})
...
Datafeeds.UDFCompatibleDatafeed.prototype._initialize = function() {
  ...
  this._send(this._datafeedURL + "/exchange/config")
    .then(function(response) {
      // var myresponse = JSON.stringify(response)
      //
      // var configurationData = JSON.parse(decodeURIComponent(myresponse));

      // console.info(configurationData)
      that._setupWithConfiguration(response);
    })
...
Datafeeds.UDFCompatibleDatafeed.prototype.resolveSymbol = function(
  ...
  if (!this._configuration.supports_group_request) {
    this._send(this._datafeedURL + "/exchange/symbols", {
      symbol: that._symbolId
    })
      .then(function(response) {
...
Datafeeds.UDFCompatibleDatafeed.prototype.getBars = function(
  ...
  // console.info("rangeEndDate - rangeStartDate"+(Number(rangeEndDate) - Number(rangeStartDate)))

  // var ftTime = Number(rangeEndDate) - Number(rangeStartDate);

  var that = this;

  var requestStartTime = Date.now();

  // console.info("rangeStartDate=====================================",rangeStartDate)
  if (!resolution) {
    // console.info("4===================================================",window.klineData)
    var data = {};
    const klineData = that._appInstance.$store.state.klineDate;
    if (klineData) {
      data = klineData;
    }
    // console.info("4===data================================================",data)
    if (JSON.stringify(data.t) == "{}" || data.code == 0) {
      data.s = "no_data";
    }
    ...
    // console.info("bars:"+JSON.stringify(bars))

    onDataCallback(bars, {
      version: that._protocolVersion,
      noData: bars.length === 0 ? true : nodata,
      nextTime: data.nb || data.nextTime
    });
  } else {
    this._send(that._klineUrl + "/charts.php", {
      market_id: symbolInfo.ticker,
      resolution: resolution === "D" ? "5D" : resolution,
      from: rangeStartDate,
      to: rangeEndDate
    })
      .then(function(response) {
        ...
        // window.klineConfig = wsConfig;
        // console.info("1==================================================="+window.sessionStorage.getItem("nowPairId")+"|"+window.localStorage.getItem("interval")+"|"+window.wsocket)
        // if (window.wsocket.readyState === 1)
        //   window.wsocket.send(JSON.stringify(wsConfig));
        // else {
        ...
        //   }, 500);
        // }
        that._appInstance.$store.commit("updateKlineConfig", wsConfig);
        that._appInstance.$store.dispatch("wsSendKline");
      })
      .catch(function(arg) {
      ...
      .then(
        (function(exchange) {
          return function(response) {
            that._onExchangeDataReceived(exchange, JSON.parse(response.data));
...
Datafeeds.DataPulseUpdater = function(datafeed, updateFrequency) {
  ...
  var update = function() {
    // console.info("2==============================",that._requestsPending)
      ...
      (function(_subscriptionRecord) {
        that._datafeed.getBars(
          ...
          function(bars) {
            ...
            // console.info("bars2:"+JSON.stringify(bars))
  ...
  if (typeof updateFrequency != "undefined" && updateFrequency > 0) {
    var vId = setInterval(update, updateFrequency);
    TradingIntervals.push(vId);
  }
};
...
Datafeeds.DataPulseUpdater.prototype.periodLengthSeconds = function(
  ...
  } else if (resolution == "5D" || resolution === "D") {
