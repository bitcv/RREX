...
(function() {
  ...
  var TradingView = {
    host:
      window.location.host.match(
        /eotpro\.com|mrn\.eotpro\.net|tradingview\.com|pyrrosinvestment\.com/i
      ) == null
    ...
    MediumWidget: function(options) {
      ...
      this.options = {
        ...
        height:
          TradingView.WidgetAbstract.prototype.fixSize(options.height) || "",
        symbols: options.symbols,
        symbols_description: options.symbols_description || "",
        large_chart_url: options.large_chart_url || "",
        customer: options.customer || "",
  ...
  TradingView.widget.prototype = {
    create: function() {
      ...
      if (!this.options.noLogoOverlay && this.options.fundamental) {
        ...
        var tvLink =
          '<a href="' +
          href +
          '" rel="nofollow" target="_blank" style="color: #3BB3E4">TradingView.com</a>';
    ...
    generateUrl: function(options) {
      ...
      var url =
        ...
        (typeof options.hide_side_toolbar === "undefined"
          ? ""
          : "&hidesidetoolbar=" + (options.hide_side_toolbar ? "1" : "0")) +
        (typeof options.allow_symbol_change === "undefined"
          ? ""
          : "&symboledit=" + (options.allow_symbol_change ? "1" : "0")) +
        (typeof options.save_image === "undefined"
          ? ""
          : "&saveimage=" + (options.save_image ? "1" : "0")) +
        "&toolbarbg=" +
        ...
        (options.news_vendors
          ...
          : "") +
        (options.studies
          ? "&studies=" + encodeURIComponent(options.studies.join(""))
          : "") +
  ...
  TradingView.chart.prototype = {
    ...
    render: function() {
      ...
      return (
        '<div class="tradingview-widget" style="width: ' +
        this.options.width +
        "px; height: " +
        this.options.height +
        'px;">' +
        '<div id="' +
        this.id +
        '_actions" class="chart-actions-float">' +
        '<a id="' +
        this.id +
        '_fullscreen" class="tradingview-button"><span class="icon icon-action-zoom"></span> Full Screen</a> ' +
        '<a id="' +
        this.id +
        '_live" class="tradingview-button" target="_blank" href="https://www.tradingview.com/chart/?clone=' +
        this.options.chart +
        '">' +
  ...
  TradingView.showSignIn = function(options, callback) {
    TradingView.dependenciesManager.depends(
      ["/static/js/external/spin.min.js"],
      "authWidget",
      function() {
  ...
  TradingView.WidgetPopup.prototype = {
    create: function() {
      ...
      var url =
        ...
        "&toolbarbg=" +
        this.options.toolbar_bg.replace("#", "") +
        (this.options.theme
          ? "&theme=" + encodeURIComponent(this.options.theme)
          : "") +
        "&hidesidetoolbar=" +
        (this.options.hide_side_toolbar ? "1" : "0") +
        (this.options.locale
          ? "&locale=" + encodeURIComponent(this.options.locale)
          : "") +
        "&" +
      ...
      this.popupWindow = window.open(
        url,
        "",
        "resizable=yes,directories=no,location=no,toolbar=no,menubar=no,scrollbars=yes, top=" +
          top +
          ", left=" +
          left +
          ", width=" +
